        ...
        for (x,y,w,h) in faces:
           ...
           #    cv2.rectangle(roi_color,(ex,ey),(ex+ew,ey+eh),(0,255,0),2)
            ...
            if w>50 and h>50 and a ==1 and end - start > 0.5:
                ...
                cv2.imwrite('haarfaces/f_'+str(len(faces))+'_'+strftime('%d%m%y_%H%M%S',gmtime())+'.jpg',crop_img ) 
        for i in range(0,height,2):
            cv2.line(frame,(0,i),(int(width/4),i),(255,0,0),1)
            cv2.line(frame,(int(width*3/4),i),(width,i),(255,0,0),1)
        cv2.imshow('HAAR',crop_img)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    except Exception:
        print("error")
